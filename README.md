...
- Start the websocket server by running `npm run ws`, default port `8080` (make sure `nodemon` is unstalled, if not then change `nodemon`=>`node` in package.json file)
